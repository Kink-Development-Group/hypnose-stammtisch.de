...
class RateLimiter
{
  ...
  {
    $now = time();
    $row = Database::fetchOne('SELECT `key`, hits, period_start FROM rate_limits WHERE `key` = ?', [$key]);
    if (!$row) {
      Database::execute('INSERT INTO rate_limits (`key`, hits, period_start) VALUES (?, 1, CURRENT_TIMESTAMP)', [$key]);
    ...
    if ($periodStart + $periodSeconds <= $now) {
      Database::execute('UPDATE rate_limits SET hits = 1, period_start = CURRENT_TIMESTAMP WHERE `key` = ?', [$key]);
      return ['allowed' => true, 'remaining' => $maxHits - 1, 'reset' => $now + $periodSeconds];
    }
    $hits = (int)$row['hits'];
    if ($hits >= $maxHits) {
    ...
    Database::execute('UPDATE rate_limits SET hits = hits + 1 WHERE `key` = ?', [$key]);
