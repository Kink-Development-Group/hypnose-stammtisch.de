<script lang="ts">
  ...
  function navClasses(active: boolean): string {
    ...
    return active
      ? `${base} bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300`
      : `${base} text-gray-700 dark:text-smoke-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-700 dark:hover:text-blue-300`;
  }
</script>
...
<nav class="flex flex-col gap-1">
  {#each navItems as item (item.key)}
    {#if hasAccess(item)}
      ...
      >
        <span
          class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 transition group-hover:bg-blue-600 dark:group-hover:bg-blue-600 group-hover:text-white"
        >
          <svg
            class="h-5 w-5"
            fill="none"
